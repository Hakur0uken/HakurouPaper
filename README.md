...
Hakurou Paper 是一款面向长文档与论文写作的本地优先 Markdown 编辑器。它希望把常用写作体验做得像普通文档软件一样直接，同时始终把文稿保留为可被 Typora、VS Code、Pandoc 等工具继续使用的标准 Markdown 文件。

## 设计追求

- **本地优先**：文稿、图片和资源都保存在用户自己选择的位置；不依赖云端账户才能写作。
- **极简而不简陋**：减少常驻工具栏和装饰，用克制的细线、固定状态栏与聚焦式编辑区组织界面。
- **低门槛 Markdown**：不要求记忆全部 Markdown 语法。标题、列表、加粗、斜体、删除线和代码等常用格式可通过悬浮菜单完成。
- **长期可迁移**：图片使用相对路径，写作成果不会被锁在应用内；同一文件夹可直接交给其他 Markdown 编辑器继续打开。
- **为学术写作准备**：公式、资源管理、Pandoc 导出、引用与版本留痕将沿着可维护、可验证的路径逐步加入。

## 当前已实现

- Tauri + Rust + React 的轻量桌面应用架构。
