---
name: hakurou-word-template-export
description: "Export a HakurouPaper Markdown manuscript into a supplied DOCX template through deterministic OOXML anchors, explicit template mapping, and package validation. Use for experimental high-fidelity academic Word-template export or validating a new template; do not use for ordinary untemplated Word export or when the template's content mapping is ambiguous."
---

# Hakurou Word Template Export Skill

This directory is the project-local implementation of the **Hakurou Word
Template Export Skill**. `SKILL.md` is its reusable Agent entry point; the
.NET 8 code alongside it is a deterministic supporting helper, not a separate
generic tool. It deliberately leaves the existing Pandoc provider untouched.

Use this Skill only for experimental, high-fidelity Word-template delivery.
For normal “按当前样式导出”, use HakurouPaper's ordinary Word export instead.
...
# Developer-only regression: use a fixture in a task-specific temporary directory.
dotnet run --project .\Hakurou.WordTemplatePoc.csproj -- run-regression ..\..\pandoc\pandoc.exe <fixture.md> <output-directory>
